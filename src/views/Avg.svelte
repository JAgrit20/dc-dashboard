 ...
 import Chart from 'chart.js/auto'
  ...
  let ctx2;
	...
	async function getTeachers() {
		const res = await fetch('/api/average_duration')
	...
	var no_of_terms=0;
        ...
        for (n in data[i]){
            ...
            avg = avg+ parseInt((data[i][n].sduration).charAt(0))  
      }
  }
  //subject
	...
	var no_of_terms=0;
        ...
        for (n in data1[i]){
			no_of_terms=no_of_terms+1
            avg = avg+ parseInt((data1[i][n].sduration).charAt(0))  
      }
  }
  //subject
        ...
        for (n in data1[i]){
          ...
          console.log(n)
			if(data1[i][n].subject=="Mathematics"){
				maths=maths+1;
			}
			if(data1[i][n].subject=="Physics"){
				phy=phy+1;
			}
			if(data1[i][n].subject=="Chemistry"){
				chem=chem+1;
			}
			if(data1[i][n].subject=="Biology"){
				bio=bio+1;
			}
			// console.log(bio,maths,phy,chem)
            // avg = avg+ parseInt((data[i][n].sduration).charAt(0))  
      }
  }
  var final = avg/no_of_terms
  console.log("avg, no_of_terms,final")
  console.log(avg, no_of_terms,final)
		console.log(bio,maths,phy,chem)
