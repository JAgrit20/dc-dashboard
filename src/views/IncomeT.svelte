<script>
	...
	async function getTeachers() {
		const res = await fetch('/api/signed-teachers')
	...
	{#each teachers as teacher (teacher.uid)}
		<div class="card">
			...
			<AccordionBody id={teacher.hKey} target={teacher.cKey} parent="#income-t-accordion">
				...
				<strong>Income: </strong>{teacher.tDoubt} <em>doubts</em> * &#x20B9;{teacher.rate_per_doubt} = &#x20B9;{+teacher.rate_per_doubt * +teacher.tDoubt}
			</AccordionBody>
		</div>
	{:else}
